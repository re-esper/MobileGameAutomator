...
const TEAM = 2; // 使用队伍
const SKILLS = [[2], [2]]; // 通常战 - 萨尔2, 螺丝2
const SKILLS2 = [[2], [2], [1, 1]]; // boss战 - 萨尔2, 螺丝2, 任意单体技
...
async function setCommand(skillNo, target = -1) {
    const enemy_x_table = [636, 712, 788]; // 2/3位, 2/2位, 3/3位
    const enemy_y = 180;
            ...
            case CS_COMMAND:
                if (stageNo < 4) { // 通常战
                    for (let skill of SKILLS) {
                        await setCommand.apply(null, skill);
                    }
                }
                else { // boss战
...
async function enterCombat(stageNo) {
    ...
    if (stageNo == 1 || stageNo == 4) {
        ...
        await waitTouch("tpl1637126766509.png");
        await bot.sleep(1);
        await waitTouch("tpl1637126786336.png");
        await bot.sleep(2);
        await touchAny();
        await bot.sleep(3);
    }
